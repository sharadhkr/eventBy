...
export default function Login() {
  ...
  /* ================= CREATE RECAPTCHA ================= */
  // FIX: Always clear and recreate the verifier to avoid Firebase reuse errors
  const getRecaptcha = () => {
  ...
  /* ================= VERIFY OTP ================= */
  // FIX: Accept otpValue as a parameter to avoid stale closure when called
  // from handleOtpChange's setTimeout
  const verifyOTP = useCallback(async (otpValue) => {
    ...
    try {
      ...
      // FIX: Log the actual error for easier debugging
  ...
  const handleOtpChange = (index, value) => {
    ...
    // FIX: Pass the fully-assembled OTP string directly instead of relying
    // on otp state (which would be stale inside the setTimeout closure)
    if (index === 5 && digit && newOtp.every((d) => d)) {
  ...
  const handleOtpKeyDown = (index, e) => {
    ...
    // FIX: Removed the Ctrl+V block — it conflicted with the onPaste handler
    // and broke native paste on some browsers/OS combos
  };
  ...
  const handleOtpPaste = (e) => {
    ...
    if (nextEmptyIndex !== -1) {
      ...
      otpRefs.current[5]?.focus();
      if (pastedData.length === 6) {
        // FIX: Pass assembled string directly, same closure-safety fix
        setTimeout(() => verifyOTP(newOtp.join("")), 300);
      }
    }
  };
  ...
  const handleBackToPhone = () => {
    ...
    setOtp(Array(6).fill(""));
    setConfirmation(null);
    // FIX: Clear the resend timer so it doesn't ghost-run in the background
    setResendTimer(0);
  };
          ...
          <div className="p-8 space-y-6">
            ...
            {step === "otp" && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right duration-500">
                ...
                {/* FIX: Uses handleBackToPhone which also clears resendTimer */}
                <button
